...
const parseRowFn     = fns.httpsCallable('parseRow');
...
function renderOpEx() {
	...
	if (!depts.length) {
		...
		td.style.cssText =
			"text-align:center;padding:2rem;color:var(--color-text-tertiary);font-size:13px;";
		tr.appendChild(td);
		tbody.appendChild(tr);
		return;
	}
...
function switchTool(toolId) {
	document
		.querySelectorAll(".nav-item[data-tool]")
		.forEach((el) => el.classList.toggle("active", el.dataset.tool === toolId));
		...
		.onSnapshot(
			(snap) => {
				if (snap.exists) {
					...
					rows = d.rows || [];
					nextId =
						d.nextId || rows.reduce((m, r) => Math.max(m, r.id), 0) + 1;
				} else {
...
document
	...
	.addEventListener("click", () => setAuthMode(!isSignUp));
...
// ── Natural language row entry ──
const nlInput = document.getElementById('nl-input');
const nlBtn   = document.getElementById('nl-btn');

async function addRowFromNL() {
  const text = nlInput.value.trim();
  if (!text || nlBtn.disabled) return;
  ...
  try {
    const result = await parseRowFn({ text });
    ...
    console.error('AI parse error — code:', err.code, '| message:', err.message, '| details:', err.details, '| full:', err);
    const codeMap = {
      'functions/unauthenticated': 'Not signed in — please refresh and sign in again.',
      'functions/invalid-argument': 'No text provided.',
      'functions/internal': 'Function error — check Firebase Function logs.',
      'functions/unavailable': 'Service unavailable — try again in a moment.',
      'functions/not-found': 'Function not found — may still be deploying.',
    };
    const msg = codeMap[err.code] || `${err.code || 'Error'}: ${err.message || 'Something went wrong'}`;
    errEl.textContent = msg;
    setTimeout(() => { errEl.textContent = ''; }, 8000);
  } finally {
    nlBtn.disabled    = false;
    nlBtn.textContent = 'Add with AI';
  }
}

nlBtn.addEventListener('click', addRowFromNL);
nlInput.addEventListener('keydown', e => { if (e.key === 'Enter') addRowFromNL(); });
